...
theme_set(theme_bw())
for (y in 1:9) {
    plot(ggplot() + geom_bar(data = pop[year == years[y]], mapping = aes(x = age, y = value, group = sex, fill = sex), stat = "identity", position = "identity") + coord_flip() + labs(x = "Age", y = "Population (thousands)", title = years[y]) + scale_y_continuous(labels = comma, limits = c(-400, 400)) + theme_bw())
}
```
